...
useEffect(() => {
  ...
  <div className="container px-5 py-20 mx-auto">
    <div className="flex pl-4 mt-4 lg:w-2/3 w-full mx-auto mb-3">
      <a href="/add_employee" className="flex ml-auto text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded">Add Employee</a>
    </div>
        ...
        <tbody>
          {employees != null && employees.length > 0 ? (
            employees.map((employee) => (
              <EmployeeListItem key={employee.id} firstname={employee.firstName} lastname={employee.lastName} email={employee.emailId} id={employee.id} />
          ))
          ): "Loading."}
        </tbody>
      </table>
