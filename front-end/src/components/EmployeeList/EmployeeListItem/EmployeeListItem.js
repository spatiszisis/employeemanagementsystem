    ...
    <tr key={id}>
      ...
      <td className='border-b-2 border-gray-200 px-4 py-3 text-lg text-gray-900'>
        <div className='flex flex-row justify-center'>
          <button
            onClick={onDelete}
            className='bg-red-600 flex flex-row items-center mr-3 px-2 rounded-md text-white cursor-pointer'
          >
            <svg
          ...
          <Link
            to={`/edit_employee/${id}`}
            className='bg-gray-400 flex flex-row items-center mr-3 px-2 rounded-md text-white cursor-pointer'
          >
            <svg
              xmlns='http://www.w3.org/2000/svg'
              className='h-6 w-6'
